...
def convert_model(cfg):
    ...
    model_path = f"{root_dir}/models/epoch=2-step=201.ckpt"
    logger.info(f"Loading pre-trained model from: {model_path}")
    cola_model = ColaModel.load_from_checkpoint(model_path).to("cpu")
    ...
    torch.onnx.export(
        cola_model,  # model to run
        (
            input_sample["input_ids"],
            input_sample["attention_mask"],
        ),  # model input (or a tuple for multirun inputs)
